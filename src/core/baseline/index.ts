...
// 杩欓噷鍘熷垯涓婄粰鍘氬害鍔犳潈baseline棰濆澧炲姞浜嗕笉纭畾鎬ф潈閲?->楂樹笉纭畾鎬у帤灞傛洿澶氱壍寮昩aseline
...
function derivativeSeries(values: number[]): number[] {
...
function optimizeScaleCoefficients(
  ...
  const coefficients = new Array<number>(bases.length).fill(0);
...
function seriesMeanSlope(values: number[]): number {
  if (values.length <= 1) {
    return 0;
  }
  let acc = 0;
...
function seriesCurvature(values: number[]): number {
  if (values.length <= 2) {
    return 0;
  }
  let acc = 0;
...
function maxDerivative(values: number[]): number {
  let out = 0;
  for (let t = 1; t < values.length; t += 1) {
