...
import GlobalStreamView from "../components/GlobalStreamView.vue";
import InsetDetailView from "../components/InsetDetailView.vue";
import MetricsView from "../components/MetricsView.vue";
import OptimizingVariantControls from "../components/OptimizingVariantControls.vue";
import SpaghettiView from "../components/SpaghettiView.vue";
...
const startupBundle = createDataGeneratorBundle(defaultState.generatorLayerCount, defaultState.generatorTimeCount);
const datasetCache = new Map<DatasetKind, DatasetBundle>();
const activeKind = ref<DatasetKind>(state.datasetKind);
const activeBundle = ref<DatasetBundle>(startupBundle);
...
async function ensureDataset(kind: DatasetKind): Promise<DatasetBundle> {
  ...
  const bundle = kind === "covid" ? await loadCovidBundle() : await loadSineBankBundle();
...
async function activateDataset(kind: DatasetKind): Promise<void> {
  ...
  try {
    ...
    resolvedKind = "dataGenerator";
    bundle = await ensureDataset("dataGenerator");
    state.datasetKind = "dataGenerator";
  }
