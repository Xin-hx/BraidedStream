...
import { buildPidTimeScene } from "../core/pid";
...
watch(
  () => [
    ...
    props.state.pidTimeOrderMode,
    props.state.pidTimeAlpha,
...
function renderChart(): void {
  ...
  if (!dataset) {
    renderPidNewEmpty(svgRef.value, "No data available for PID time");
    return;
  }

  const scene = buildPidTimeScene({
    dataset,
    roi: props.roi,
    ...
    orderMode: props.state.pidTimeOrderMode,
    alpha: props.state.pidTimeAlpha
  });
