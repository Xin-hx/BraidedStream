import { clamp, median, normalize01, sumAbs } from "./utils.js";
import { layerUncertaintyAt, validateTimeLengths } from "./validate.js";
...
export function computeUncertaintyAwareBaseline(times, layers, strength = 0.45) {
    validateTimeLengths(times, layers);
    ...
    const unc = layers.map((layer) => {
        const arr = new Array(tLength).fill(0);
    ...
    const weights = Array.from({ length: kLength }, () => new Array(tLength).fill(0));
    for (let k = 0; k < kLength; k += 1) {
        for (let t = 0; t < tLength; t += 1) {
    ...
    const fPrime = Array.from({ length: kLength }, () => new Array(tLength).fill(0));
    for (let k = 0; k < kLength; k += 1) {
        for (let t = 1; t < tLength; t += 1) {
            fPrime[k][t] = layers[k].mean[t] - layers[k].mean[t - 1];
        }
    }
    const gPrime = new Array(tLength).fill(0);
    for (let t = 1; t < tLength; t += 1) {
        let numerator = 0;
        let denominator = 0;
        let prefix = 0;
...
export function computeMultiscaleDistributedBaseline(times, layers, strength = 0.45, hooks = {}, energyThreshold = 0.08) {
    ...
    const anchorCenterlineDerivative = derivativeSeries(anchorCenterline);
...
function sumLayerMeans(tLength, layers) {
...
function computeWiggleBaseline(tLength, layers, mode, hooks) {
...
function centeredBaselineFromLayers(tLength, layers) {
    const totals = sumLayerMeans(tLength, layers);
    return totals.map((v) => -0.5 * v);
}
function buildCenterLineDerivativeOffsets(tLength, layers) {
...
function derivativeSeries(values) {
...
function baselineOptimizationMetrics(layers, baseline) {
    ...
    return {
        ...
        centerlineMeanSlope: seriesMeanSlope(centerline),
        maxBaselineDerivative: maxDerivative(centerline),
        derivativeConcentration: derivativeConcentration(centerline),
        centerlineCurvature: seriesCurvature(centerline),
...
function seriesMeanSlope(values) {
    if (values.length <= 1) {
        return 0;
    }
    let acc = 0;
...
function seriesCurvature(values) {
    if (values.length <= 2) {
        return 0;
    }
    let acc = 0;
...
function slopeCoverage(values) {
    const maxValue = maxDerivative(values);
 ...
 */
function computeSineStreamBaseline(tLength, layers, hooks) {
