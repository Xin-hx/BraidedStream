...
function buildLayerBandSeries(layer, tLength, uncertaintySource) {
    const controlPoints = CONTOUR_MASK_MEMBERSHIP.map((item) => ({
        ...
        values: new Array(tLength).fill(0)
    }));
...
function buildContourMaskMembershipSpec() {
    const byKey = new Map();
...
function resolveQuantileMaskValues(layer, timeIndex, meanValue, uncertaintySource) {
    const quantiles = quantilesForSource(layer, uncertaintySource);
    const uncertainty = Math.max(0, finiteOr(uncertaintySource === "poportion" ? layer.poportionUnc?.[timeIndex] : layer.unc?.[timeIndex], 0));
    const p025 = firstFinite(quantiles?.p025?.[timeIndex], quantiles?.p05?.[timeIndex], quantiles?.p10?.[timeIndex], uncertaintySource === "poportion" ? layer.poportionLower?.[timeIndex] : layer.lower?.[timeIndex], meanValue - 0.5 * uncertainty, meanValue);
    const p975 = firstFinite(quantiles?.p975?.[timeIndex], quantiles?.p95?.[timeIndex], quantiles?.p90?.[timeIndex], uncertaintySource === "poportion" ? layer.poportionUpper?.[timeIndex] : layer.upper?.[timeIndex], meanValue + 0.5 * uncertainty, meanValue);
...
function rasterizeBand(band, grid) {
    ...
    for (let t = 0; t < grid.xBins; t += 1) {
        const controlPoints = prepareMembershipPoints(band.controlPoints.map((point) => [transformValue(point.values[t], grid.valueTransform), point.membership]));
...
function prepareMembershipPoints(points) {
    const sorted = points
        .filter(([x, y]) => Number.isFinite(x) && Number.isFinite(y))
        .map(([x, y]) => [x, clamp(y, 0, 1)])
        .sort((a, b) => a[0] - b[0]);
    const collapsed = [];
