...
export function computeMultiscaleDistributedBaseline(times, layers, strength = 0.45, hooks = {}, energyThreshold = 0.08) {
    ...
    const saliencyDenom = new Array(tLength).fill(0);
    for (const band of bands) {
        for (let t = 0; t < tLength; t += 1) {
    ...
    const uncertaintySaliencyRaw = new Array(tLength).fill(0);
    ...
    const distributedShift = new Array(tLength).fill(0);
    for (let b = 0; b < bands.length; b += 1) {
        const band = bands[b];
        const weighted = new Array(tLength).fill(0);
        for (let t = 0; t < tLength; t += 1) {
...
function aggregateUncertaintyVariation(layers, tLength) {
...
function scaleToBudget(values, targetBudget) {
...
function computeGaussianWeightedAdjustment(layers, i, c) {
    ...
    const dFi = new Array(n); // thickness changes
    const Qi = new Array(n); // cumulative contributions
